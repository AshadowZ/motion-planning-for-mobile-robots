function [Aeq, beq] = getAbeq(n_seg, n_order, ts, start_cond, end_cond)
    ...
    Aeq_start(1,1) = 1*s^(1-0); % 位置
    Aeq_start(2,1:2) = n_order * [-1,1] * s^(1-1); % 速度
    Aeq_start(3,1:3) = n_order * (n_order-1) * [1, -2, 1]* s^(1-2); % 加速度
    ...
    Aeq_end(1,end) = 1 * s^(1-0); % 位置
    Aeq_end(2,end -1:end) = n_order * [-1,1] * s^(1-1); % 速度
    Aeq_end(3,end -2:end) = n_order * (n_order-1) * [1, -2, 1] * s^(1-2); % 加速度
    beq_end = end_cond';
