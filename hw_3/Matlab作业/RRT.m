...
for iter = 1:3000
    ...
    %提示：x_near已经在树T里
    distArray = arrayfun(@(x) getDist(x_rand(1),x_rand(2),x.x, x.y),T.v); %向量化运算啦，懒得写了
    [~,idx] = min(distArray);
    ...
    x_new = [x_near(1)+Delta*cos(theta),x_near(2)+Delta*sin(theta)]; %简单的运算
