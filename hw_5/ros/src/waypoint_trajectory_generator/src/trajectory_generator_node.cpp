...
void trajGeneration(Eigen::MatrixXd path)
{
    ...
    _polyCoeff = trajectoryGeneratorWaypoint.PolyQPGeneration(_dev_order, path, vel, acc, _polyTime, 1);
...
{ 
    ...
    */
    // Path是n*3的matrix，每一行都是一个waypoint 
    double acc_time = _Vel / _Acc; // 加速度所需时间
    double acc_dis = _Acc * acc_time * acc_time; // 两端加减速的距离
    double dis;
    for(int k = 0; k < Path.rows()-1; k++){ // 高中数学
        dis = (Path.row(k+1) - Path.row(k)).norm();
        if(dis < acc_dis){ // 如果都加不到_Vel就跑完了
            time(k) = std::sqrt(dis / _Acc);
        }
        else{
            time(k) = acc_time + (dis - acc_dis) / _Vel;
        }
    }
